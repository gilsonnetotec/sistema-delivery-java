...
public class CategoryResourceTests {
    ...
    public void getShouldReturnEntityWhenSearchForOneId() {
        ...
        Assertions.assertEquals(exintingId, result.getId());
    ...
    public void getShouldNotReturnEntityWhenCategory_NotFound(){
        ...
        Assertions.assertThrows(ResourceNotFoundException.class, ()-> service.findById(exintingId));
    ...
    public void updateShouldPersistWithAutoincrementWhenHasId(){
        ...
        Mockito.when(repository.getOne(exintingId)).thenReturn(entity);
        Mockito.when(repository.save(ArgumentMatchers.any())).thenReturn(entity);

        dto.setName("Nova Categoria");

        CategoryDTO result = service.update(exintingId, dto);
    ...
    public void updateShouldNotPersistWhenCategory_NotFound(){
        Mockito.when(repository.getOne(exintingId)).thenThrow(EntityNotFoundException.class);

        dto.setName("Nova Categoria");

        Assertions.assertThrows(ResourceNotFoundException.class, () -> service.update(exintingId, dto));
